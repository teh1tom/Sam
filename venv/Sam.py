...
class Main(discord.Client):
    async def on_message(self, message):
        if message.author != self.user:
            print(message.content.upper(), message.author.name.upper(), str(message.author.id))
            output = Command.input(message.author.id, message.author.name.upper(), message.content.upper())
